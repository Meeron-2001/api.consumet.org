...
const routes = async (fastify: FastifyInstance, options: RegisterOptions) => {
  const createZoro = async () => {
    const mod = await import('@consumet/extensions/dist/providers/anime/zoro');
    const Zoro = (mod as any).default || (mod as any);
    return new Zoro(process.env.ZORO_URL);
  };
